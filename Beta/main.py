...
def load_model():
    ...
    global model
    model_path = 'random_forest_model-2.pkl'  # Загрузка модели по умолчанию при запуске
...
def real_time_analysis():
    ...
    if trained_model is None:
        messagebox.showerror("Ошибка", "Неверный формат сохраненной модели. Проверьте файл модели.")
        return
    ...
    while True:
        ...
        try:
            ...
            # Проверяем, что все необходимые столбцы присутствуют
            required_columns = ['IN_BYTES', 'OUT_BYTES', 'IN_PKTS', 'OUT_PKTS', 'FLOW_DURATION_MILLISECONDS', 'SRC_PORT', 'DST_PORT']
            for col in required_columns:
                if col not in logs_df.columns:
                    messagebox.showerror("Ошибка", f"Столбец '{col}' отсутствует в данных.")
                    return
            ...
            # Прогнозируем атаки
            data = logs_df[required_columns]
            logs_df['Prediction'] = trained_model.predict(data)
